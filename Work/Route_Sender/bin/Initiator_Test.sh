...
USDFiles=$FLUTESender/1b_1M_test/\{MultiRate_Dynamic.mpd,BBB_720_1M_video_init.mp4,BBB_32k_init.mp4,../SDP.sdp\}
...
encodingSymbolsPerPacket=0	#For Receiver This means that more than one is included. This could be varying
...
#(cd $FLUTEReceiver && valgrind --tool=memcheck --leak-check=full  --show-leak-kinds=all ./flute -A -B:$DASHContent -b:2 -Q -d:$sdp -v:4 -J:$Log)
(cd $FLUTEReceiver && ./flute -A -B:$DASHContent -d:$sdp -v:4 -Q -Y:$encodingSymbolsPerPacket -J:$Log)
#(cd $FLUTEReceiver && valgrind --tool=memcheck  --leak-check=full --show-leak-kinds=all ./flute -A -B:$DASHContent -Q -d:$sdp -v:4 -J:$Log)
#(cd $FLUTEReceiver && valgrind --tool=memcheck --leak-check=full ./flute -A -B:$DASHContent -Q -d:$sdp2 -v:4 -J:$Log2)
#(cd $FLUTEReceiver && env HEAPCHECK=normal ./flute -A -B:$DASHContent -Q -d:$sdp2 -v:4 -J:$Log2)
