...
awk -v mpdLen=${mpd[0]} -v mpdNam=${mpd[1]} -v vidLen=${vidInit[0]} -v vidNam=${vidInit[1]} -v audLen=${audInit[0]} -v audNam=${audInit[1]} -v vidFile='efdt_Video.xml' -v audFile='efdt_Audio.xml' -v id=$(($TOI_Video)) -v idAud=$(($TOI_Audio)) '{if (index($0,"video_8M_")) {print "<File TOI=\""id"\" Content-Location=\"file:///"vidNam"\" Content-Length=\""vidLen"\"/>" >> vidFile; id++;print "<File TOI=\""id"\" Content-Location=\"file:///"$1"\" Content-Length=\""$2"\"/>" >> vidFile; id++} else if (index($0,"audio_64k")){print "<File TOI=\""idAud"\" Content-Location=\"file:///"mpdNam"\" Content-Length=\""mpdLen"\"/>" >> audFile;idAud++;print "<File TOI=\""idAud"\" Content-Location=\"file:///"audNam"\" Content-Length=\""audLen"\"/>" >> audFile;idAud++;print "<File TOI=\""idAud"\" Content-Location=\"file:///"$1"\" Content-Length=\""$2"\"/>" >> audFile;idAud++}}' files.txt
