...
/*This is used to configure how many encoding symbols to send per packet. Only tested with Compact-No-Code-FEC*/
/*IMPORTANT: es_len*nb_of_symbols SHOULD NOT lead to packet sizes exceeding maximum transmission unit*/
unsigned short nb_of_symbols;
...
int alc_send(int s_id, int tx_mode, char *buf, int buf_len,
  ...
  if (numEncSymbPerPacket == 0 && toi !=FDT_TOI /* && sendingTimesFile !="" */)
  {
    if (getNextLine)
    {
