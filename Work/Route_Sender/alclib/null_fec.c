...
trans_block_t* null_fec_encode_src_block(char *data, unsigned long long len,
	...
	unsigned short skippedData=0;
	//Put metaData units at the end of the transport units list
	unsigned int nb_of_units_metaData;
	nb_of_units_metaData = (unsigned int)ceil((double)(unsigned int)segmentMetaData / (double)es_len);
	//End
	for(i = 0; i < nb_of_units; i++) {
		...
		//printf("This is the current encoding symbol id: %u\n", i);
		///End
	...
	//Start
	//Now populate meta data transport units
	ptr = data;
	for(i = 0; i < nb_of_units_metaData; i++) {

			tr_unit->esi = i;
			tr_unit->len =  skippedData < es_len ? (unsigned short)skippedData : es_len;
