...
function callback(e)
{
  ...
  window.WebSocket = window.WebSocket || window.MozWebSocket;
		...
		var arrayLength = 0;
		var initVideoSegmentAppended = false;
		var initAudioSegmentAppended = false;
		...
		{				
			...
			fileReader.onload = function() 
			{
				...
				if ( (arrayLength == 900 || arrayLength == 920 ) && initVideoSegmentAppended ){
					return "Video init segment already appended.";
				}
				else {			
					initVideoSegmentAppended = true;
				}
				
				//console.log("Length of video array = " + arrayData.length);				
				
				if(initVideoBuffer == true)
				{
//					console.log("Intilization length of array = " + arrayData.length);
					videoBuffer = arrayData;
                    initVideoBuffer = false;				
                }
				else 
				{	
					//else if (arrayLength != 900 && arrayLength != 920 )
        ...
        websocketAudio.onmessage = function (message) {
			...
			fileReader.onload = function() {
				...
				if ( (arrayLength == 826)  && initAudioSegmentAppended ){
					return "Audio init segment already appended.";
				}
				else {			
					initAudioSegmentAppended = true;
				}
				
				//console.log("Length of audio array = " + arrayData.length);						
